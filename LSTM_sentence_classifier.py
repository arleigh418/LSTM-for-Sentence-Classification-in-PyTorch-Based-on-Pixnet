...
def train():
    train_data,dev_data,test_data,word_to_ix,label_to_ix = data_loader.read_data()
    EMBEDDING_DIM = 400
    HIDDEN_DIM = 400
    EPOCH = 100
    best_dev_acc = 0.0
...
#             os.system('rm mr_best_model_acc_*.model')
            print('New Best Dev!!!')
            torch.save(model.state_dict(), 'best_model_acc' + str(int(test_acc*10000)) + '.model')
            no_up = 0
        else:
            no_up += 1
            if no_up >= 10:
...
def evaluate(model, data, loss_function, word_to_ix, label_to_ix, name ='dev'):
    ...
    for sent, label in data:
        ...
        pred = model(sent)
        pred_label = pred.data.max(1)[1].cpu().detach().numpy()
